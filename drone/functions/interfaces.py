...
def _query_all(object_type: str) -> list[dict]:
    ...
    return _load(object_type.lower() + ".json") if not any(c.isupper() for c in object_type) else _load_by_type(object_type)


def _load_by_type(object_type: str) -> list[dict]:
    result = []
    for i, ch in enumerate(object_type):
        if ch.isupper() and i > 0:
            result.append("_")
        result.append(ch.lower())
    return _load("".join(result) + ".json")
